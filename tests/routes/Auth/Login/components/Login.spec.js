...
describe('(Component) Login', () => {
  ...
  // it('Should render with an <h2> that includes Sample Login text.', () => {
  //   expect(_wrapper.find('h2').text()).to.match(/Login:/);
  // });
  ...
  describe('A login button...', () => {
    ...
    beforeEach(() => {
      _button = _wrapper.find('button').filterWhere(a => a.text() === 'Login');
    });

    // it('has material-ui classes', () => {
    //   expect(_button.hasClass('btn btn-default')).to.be.true;
    // });

    it('Should dispatch a `login` action when clicked', () => {
      _spies.dispatch.should.have.not.been.called;
